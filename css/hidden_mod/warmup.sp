...
void JoinProperTeam(int client)
{
	if(!IsClientInGame(client))
		return;
	
	int iJoin;
	int iTerror = GetTeamClientCount(CS_TEAM_T);
	int iCTerror = GetTeamClientCount(CS_TEAM_CT);
	
	if(iTerror < iCTerror)
		iJoin = CS_TEAM_T;
	else if(iCTerror < iTerror)
		iJoin = CS_TEAM_CT;
	else
		iJoin = GetRandomInt(CS_TEAM_T, CS_TEAM_CT);
	
	ChangeClientTeam(client, iJoin);
}
