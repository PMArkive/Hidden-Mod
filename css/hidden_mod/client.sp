...
void SetCurrentWeaponAlpha(int client, int alpha)
{
	for(int i = 0; i < 5; i++)
	{
		...
		if(IsValidEntity(weapon))
		{
			...
			SetEntProp(weapon, Prop_Send, "m_iWorldModelIndex", WorldModelHider);
...
void SetPlayerGhost(int client)
{
	if(IsPlayerAlive(client))
	{		
		SetEntityModel(client, hidden_model);
...
// 고스트 서로 팀바꾸기
void SwitchGhost(int oldHidden, int newHidden)
{
	if(ConnectionCheck(newHidden))
		CS_SwitchTeam(newHidden, 2);
	if(ConnectionCheck(oldHidden))	
...
{
	return GetEntData(client, flashbang_counter + (view_as<int>(12) * 4));
}
...
void MakePunchView(int client)
{
	...
	GetEntPropVector(client, Prop_Send, "m_vecPunchAngle", client_angle);
	...
	SetEntPropVector(client, Prop_Send, "m_vecPunchAngle", client_angle);
	SetEntPropVector(client, Prop_Send, "m_vecPunchAngleVel", new_angle);
	
//	EmitSoundToAll(hurt, client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
}
...
void PlayRandomDeathSound(int client)
{
	if(GetClientTeam(client) == 2)
	{
		...
		if(random_int == 1)
		{
			EmitSoundToAll(death1, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 2)
		{
			EmitSoundToAll(death2, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 3)
		{
			EmitSoundToAll(death3, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 4)
		{
			EmitSoundToAll(death4, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);	
		}
		if(random_int == 5)
		{
			EmitSoundToAll(death5, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 6)
		{
			EmitSoundToAll(death6, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
	}
	else
	{
		...
		if(random_int == 1)
		{
			EmitSoundToAll(ct_death, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_SCREAMING, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 2)
		{
			EmitSoundToAll(ct_death2, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_SCREAMING, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 3)
		{
			EmitSoundToAll(ct_death3, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_SCREAMING, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 4)
		{
			EmitSoundToAll(ct_death4, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_SCREAMING, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		if(random_int == 5)
		{
			EmitSoundToAll(ct_death5, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_SCREAMING, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
	}
}
