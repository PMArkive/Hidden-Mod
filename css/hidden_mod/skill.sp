				...
				if(TR_DidHit(tracer))
				{
					...
					if(GetVectorLength(distance) <= 70.0 && GetVectorLength(distance) >= 40.0)
					{
						if(lasermine[client] > 0)
						{
							...
							DispatchKeyValue(mine_entity, "model", mine_model);
							...
							EmitSoundToAll(mine_attach, mine_entity, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, 100, mine_entity, mine_pos, NULL_VECTOR, true, 0.0);
							...
							SetEntityModel(laser_entity, mine_laser);
...
{
	EmitSoundToAll(mine_sound2, SOUND_FROM_WORLD, SNDCHAN_AUTO, 90/*SNDLEVEL_SCREAMING*/, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, entity, NULL_VECTOR, NULL_VECTOR, true, 0.0);
...
void make_fake_explosion(float pos[3])
{
	...
	if(boom != -1)
	{
		...
		EmitSoundToAll(mine_sound, SOUND_FROM_WORLD, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, pos, NULL_VECTOR, true, 0.0);
	}
}
...
public Action start_beam(Handle timer, Handle data_pack)
{
	...
	if(laser_entity != -1)
	{
		...
		EmitSoundToAll(mine_active, laser_entity, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, 100, laser_entity, beam_end_pos, NULL_VECTOR, true, 0.0);
	}
...
public int OnEntityCreated(int entity, const char[] classname)
{
	...
	{
		RequestFrame(SgProjectile, entity);
	}
	
	if(StrEqual(classname, "env_particlesmokegrenade", false))
	{
		RequestFrame(SgParticles, entity);
	}
}
...
void UseAdrenaline(int client)
{
	if(adrenaline[client] > 0)
	{
		...
		EmitSoundToAll(use_adrenaline, client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
...
public Action reset_adrenaline(Handle timer, any client)
{
	if(is_using_adrenaline[client] == true)
	{
		...
		if(adrenaline[client] > 0 && IsPlayerAlive(client))
		{
			...
			EmitSoundToAll(end_adrenaline, client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		adrenaline_timer[client] = INVALID_HANDLE;
	}
}

public void SgProjectile(any entity)
{
	SetEntityModel(entity, poison_model);
...
public void SgParticles(any entity)
{
