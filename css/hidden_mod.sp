...
public void OnPluginStart()
{
	...
	hm_env_light = CreateConVar("hm_env_light", "b", "Light style");
	...
	RegAdminCmd("showhiddenloc", test_show_hidden_loc, ADMFLAG_ROOT, "ONLY FOR DEBUG!!");
...
public Action NormalSoundHook(int clients[64], int &numClients, char sample[PLATFORM_MAX_PATH], int &entity, int &channel, float &volume, int &level, int &pitch, int &flags)
{
	if (strncmp(sample, "weapons/knife/knife_deploy1.wav", 17, false) == 0)
	{
		...
		EmitSoundToAll(sample, entity, channel, level, flags, volume, pitch, _, _, _, true);
...
public void OnMapStart()
{
	...
	WorldModelHider = PrecacheModel("models/blackout.mdl", true);
...
public void OnMapEnd()
{
	...
	ClearArray(hidden_history);
	round_gamestart_avoid = false;
//	ClearTimer(round_timer); // TIMER NO MAPCHANGED! DON't USE
}
...
{
	if (ConnectionCheck(client) && IsPlayerAlive(client) && GetClientTeam(client) == 2 && !IsWarmup)
	{
	...
	if (ConnectionCheck(client) && IsPlayerAlive(client) && GetClientTeam(client) == 3)
	{
		if (!IsWarmup && !round_gamestart_avoid)
		{
			...
			if (taunt_delay[client] <= CurrentGameTime)
			{
				...
				EmitSoundToAll(iris_taunt[Rand], client, SNDCHAN_AUTO, SNDLEVEL_SCREAMING, SND_CHANGEVOL, 0.7, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
...
public Action nvgs_use(int client, int args)
{
	...
	if (GetClientTeam(client) == 2 && IsPlayerAlive(client))
	{
		if (ghost_nvgs_checker[client] == false)
		{
			...
			{
				EmitSoundToAll("ambient/office/button1.wav", client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		...
		else
		{
			...
			if (ghost_nvgs_timer[client] != INVALID_HANDLE)
			{
				EmitSoundToAll("weapons/zoom.wav", client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
...
public Action adrenaline_use(int client, int args)
{
	...
	{
		if (is_using_adrenaline[client] == false)
		{
...
public Action OnTakeDamage(int client, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if (ConnectionCheck(attacker) && ConnectionCheck(client))
	{
		...
		{
			if(StrContains(weaponName, "weapon_") != -1)
			{
				damage *= 0.65;
				PrintToChatAll("DEBUG: HIT: HUMAN -> HIDDEN %f", damage);
				return Plugin_Changed;
			}
		}
		...
		if (GetClientTeam(attacker) == 2 && GetClientTeam(client) == 3)
		{
			...
			{
				PrintToChatAll("DEBUG: HIT: HIDDEN -> HUMAN %f", damage);
				SetEntityHealth(attacker, RoundToFloor(GetClientHealth(attacker) + (damage*0.25)));
			}
		}
	}
...
public Action round_start(Event event, char[] name, bool broadcast)
{
	...
	if (!IsWarmup)
	{
		if (!round_gamestart_avoid)
		{
			...
			EmitSoundToAll(round_start_sound[Rand], SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, 0, NULL_VECTOR, NULL_VECTOR, true, 0.0);
		}
		else
		{
			/*
			...
			CreateTimer(0.0, KickBot, EnderBot);
...
public Action round_end(Event event, char[] name, bool broadcast)
{
	...
	*/
	...
	{
		if (!round_gamestart_avoid) // DEBUG TODO
		{
			if (GetTeamClientCount(2) + GetTeamClientCount(3) > 1)
			{
				PickRandomGhost = true;
	...
	EmitSoundToAll(round_end_sound[Rand], SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_CHANGEVOL, 1.0, SNDPITCH_NORMAL, 0, NULL_VECTOR, NULL_VECTOR, true, 0.0);
}
...
public Action push_player(Handle timer, any client)
{
	...
	if (angle[0] < 0.0)
	{
		...
		EmitSoundToAll(jump, client, SNDCHAN_AUTO, SNDLEVEL_SCREAMING, SND_NOFLAGS, 1.0, SNDPITCH_NORMAL, client, NULL_VECTOR, NULL_VECTOR, true, 0.0);
	}
}
